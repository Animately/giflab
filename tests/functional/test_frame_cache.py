...
class TestFrameCacheEntry:
    ...
    def test_entry_serialization(self, sample_frames):
        ...
        for orig_frame, restored_frame in zip(
            original_entry.frames, restored_entry.frames, strict=True
        ):
...
class TestFrameCache:
    ...
    def test_cache_disabled(self, temp_cache_dir, sample_frames, sample_gif):
        ...
        cache = FrameCache(
            memory_limit_mb=10, disk_path=temp_cache_dir / "test.db", enabled=False
        )
    ...
    def test_cache_file_change_invalidation(self, frame_cache, sample_frames, tmp_path):
        ...
        # Cache it
        frame_cache.put(
            gif_path, sample_frames, frame_count=1, dimensions=(100, 100), duration_ms=0
        )
    ...
    def test_disk_cache_persistence(self, temp_cache_dir, sample_frames, sample_gif):
        ...
        cache1 = FrameCache(
            memory_limit_mb=10, disk_path=temp_cache_dir / "persist.db", enabled=True
        )
...
class TestCacheStats:
    ...
    def test_cache_stats_hit_rate(self):
        ...
        stats = CacheStats(hits=75, misses=25, total_accesses=100)
