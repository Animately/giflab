...
class TestDecorators:
    ...
    def test_track_timing_decorator(self):
        ...
        with patch(
            "giflab.monitoring.decorators.get_metrics_collector", return_value=collector
        ):
...
class TestAlerting:
    ...
    def test_alert_manager_rules(self):
        ...
        with patch(
            "giflab.monitoring.alerting.get_metrics_collector", return_value=collector
        ):
        ...
        assert len(alerts) > 0
        cache_alerts = [
            a for a in alerts if "cache" in a.system and "hit_rate" in a.metric
        ]
