...
class TestAdaptiveSampler:
    ...
    def test_adaptive_metadata(self):
        ...
        frames = [
            np.random.randint(0, 256, (10, 10, 3), dtype=np.uint8) for _ in range(30)
        ]
...
class TestProgressiveSampler:
    ...
    def test_progressive_sampling_basic(self):
        ...
        frames = [
            np.random.randint(0, 256, (10, 10, 3), dtype=np.uint8) for _ in range(50)
        ]
    ...
    def test_progressive_with_metric(self):
        ...
        frames = [np.ones((10, 10, 3), dtype=np.uint8) * i for i in range(30)]
    ...
    def test_progressive_convergence(self):
        ...
        frames = [
            np.random.randint(0, 256, (10, 10, 3), dtype=np.uint8) for _ in range(100)
        ]
...
class TestSceneAwareSampler:
    ...
    def test_scene_detection(self):
        ...
        sampler = SceneAwareSampler(scene_threshold=0.3, min_frames_threshold=10)
    ...
    def test_scene_boundary_sampling(self):
        ...
        sampler = SceneAwareSampler(
            scene_threshold=0.3, boundary_window=3, min_frames_threshold=10
        )
...
class TestSamplerFactory:
    ...
    def test_create_uniform_sampler(self):
        ...
        sampler = create_sampler(SamplingStrategy.UNIFORM, sampling_rate=0.5)
        assert isinstance(sampler, UniformSampler)

    def test_create_adaptive_sampler(self):
        """Test creating adaptive sampler via factory."""
        sampler = create_sampler(SamplingStrategy.ADAPTIVE, base_rate=0.3)
    ...
    def test_create_progressive_sampler(self):
        ...
        sampler = create_sampler(SamplingStrategy.PROGRESSIVE, initial_rate=0.15)
    ...
    def test_create_scene_aware_sampler(self):
        ...
        sampler = create_sampler(SamplingStrategy.SCENE_AWARE, scene_threshold=0.25)
