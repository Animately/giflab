...
class BenchmarkValidationCache:
    ...
    def generate_test_frames(
        num_frames: int = 10, size: tuple[int, int] = (100, 100)
    ) -> list[np.ndarray]:
    ...
    def benchmark_metric_caching(self, temp_cache_dir: Path):
        ...
        with patch("giflab.caching.metrics_integration.VALIDATION_CACHE", config):
            with patch("giflab.config.VALIDATION_CACHE", config):
                ...
                with patch(
                    "giflab.metrics.calculate_ms_ssim", side_effect=mock_ms_ssim
                ):
                    with patch(
                        "skimage.metrics.structural_similarity", side_effect=mock_ssim
                    ):
                        with patch(
                            "giflab.deep_perceptual_metrics.calculate_lpips_frames",
                            side_effect=mock_lpips,
                        ):
                            # Reset cache
                            reset_validation_cache()
        ...
        return {
            "ms_ssim": {
                ...
                "speedup": np.mean(ms_ssim_times["first"])
                / np.mean(ms_ssim_times["cached"]),
            },
            "ssim": {
            ...
            "lpips": {
                ...
                "speedup": np.mean(lpips_times["first"])
                / np.mean(lpips_times["cached"]),
            },
        }
    ...
    def run_all_benchmarks(self, temp_cache_dir: Path) -> dict[str, Any]:
        ...
        print(
            f"   Get (hit) avg: {results['basic_operations']['get_hit_avg_ms']:.2f}ms"
        )
        print(
            f"   Get (miss) avg: {results['basic_operations']['get_miss_avg_ms']:.2f}ms"
        )
...
def test_validation_cache_performance():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            results["basic_operations"]["get_hit_avg_ms"] < 1.0
        ), "Cache hits should be < 1ms"
        assert results["basic_operations"]["hit_rate"] > 0.9, "Hit rate should be > 90%"

        # Assert speedups
        assert (
            results["metrics"]["ms_ssim"]["speedup"] > 5
        ), "MS-SSIM should have > 5x speedup"
        assert (
            results["metrics"]["ssim"]["speedup"] > 3
        ), "SSIM should have > 3x speedup"
        assert (
            results["metrics"]["lpips"]["speedup"] > 10
        ), "LPIPS should have > 10x speedup"
