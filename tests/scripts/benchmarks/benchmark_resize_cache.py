...
class ResizeCacheBenchmark:
    ...
    def generate_test_frames(
        self, size: tuple[int, int], count: int = 10
    ) -> list[np.ndarray]:
        """Generate test frames of specified size."""
        h, w = size
        return [
            np.random.randint(0, 255, (h, w, 3), dtype=np.uint8) for _ in range(count)
        ]

    def benchmark_single_resize(
        self,
        name: str,
    ...
    def benchmark_buffer_pooling(self):
        ...
        pool_stats = (
            cache_with_pool._buffer_pool.get_stats()
            if cache_with_pool._buffer_pool
            else {}
        )
        ...
        if pool_stats:
            console.print(
                f"  Buffer reuse rate: [cyan]{pool_stats['reuse_rate']:.1%}[/cyan]"
            )
    ...
    def print_results(self):
        ...
        if avg_speedup > 10:
            console.print(
                "  ✅ [green]Excellent cache performance - significant speedup achieved[/green]"
            )
        elif avg_speedup > 5:
            console.print(
                "  ✅ [green]Good cache performance - notable speedup achieved[/green]"
            )
        elif avg_speedup > 2:
            console.print(
                "  🟡 [yellow]Moderate cache performance - some benefit from caching[/yellow]"
            )
        else:
            console.print(
                "  🔴 [red]Limited cache benefit - consider profiling usage patterns[/red]"
            )
        ...
        if total_memory < 50:
            ...
            console.print(
                "  🔴 [red]High memory usage - consider adjusting cache limits[/red]"
            )
...
def main():
    ...
    with Progress(
        ...
        benchmark.benchmark_single_resize(
            "Small frames (200x200 → 100x100)", (200, 200), (100, 100)
        )
        progress.update(task, advance=1)

        benchmark.benchmark_single_resize(
            "Medium frames (600x600 → 300x300)", (600, 600), (300, 300)
        )
        progress.update(task, advance=1)

        benchmark.benchmark_single_resize(
            "Large frames (1200x1200 → 600x600)", (1200, 1200), (600, 600)
        )
        progress.update(task, advance=1)
