...
class ComprehensiveBenchmarkSuite:
    ...
    def run_baseline_benchmark(self, scenario: BenchmarkScenario) -> BenchmarkResult:
        ...
        try:
            ...
            metrics = calculate_comprehensive_metrics(
                original_frames=frames_orig, compressed_frames=frames_comp
            )
    ...
    def run_optimized_benchmark(self, scenario: BenchmarkScenario) -> BenchmarkResult:
        ...
        try:
            ...
            if quality_tier == "HIGH":
                ...
                metrics_calculated = [
                    k for k in metrics.keys() if k not in metrics_skipped
                ]
            else:
                metrics_calculated = list(metrics.keys())
                metrics_skipped = []
    ...
    def run_parallel_only_benchmark(
        self, scenario: BenchmarkScenario
    ) -> BenchmarkResult:
        ...
        try:
            ...
            ] = "true"  # Keep cache for fair comparison
    ...
    def run_conditional_only_benchmark(
        self, scenario: BenchmarkScenario
    ) -> BenchmarkResult:
    ...
    def run_comprehensive_benchmark(self) -> dict:
        ...
        for i, scenario in enumerate(self.scenarios, 1):
            ...
            print(
                f"  Frames: {scenario.frame_count}, Resolution: {scenario.resolution}"
            )
            print(
                f"  Quality: {scenario.quality_level}, Content: {scenario.content_type}"
            )
            ...
            if baseline_result.execution_time > 0:
                parallel_speedup = (
                    baseline_result.execution_time / parallel_result.execution_time
                )
                conditional_speedup = (
                    baseline_result.execution_time / conditional_result.execution_time
                )
                full_speedup = (
                ...
                comparison = {
                    ...
                    "memory_reduction": (
                        baseline_result.memory_peak - optimized_result.memory_peak
                    )
                ...
                if optimized_result.metrics_skipped:
                    print(
                        f"    Metrics skipped: {', '.join(optimized_result.metrics_skipped)}"
                    )
    ...
    def _generate_report(self, results: dict, output_file: Path):
        ...
        with open(output_file, "w") as f:
            ...
            for comparison in results.get("comparisons", []):
                ...
                f.write(
                    f"    Conditional Only: {comparison['conditional_speedup']:.2f}x\n"
                )
            ...
            if summary:
                ...
                for level, stats in summary.get("speedup_by_quality", {}).items():
                    if stats["count"] > 0:
                        f.write(
                            f"  {level.capitalize()}: {stats['mean']:.2f}x average speedup ({stats['count']} scenarios)\n"
                        )
                ...
                for size, stats in summary.get("speedup_by_size", {}).items():
                    if stats["count"] > 0:
                        f.write(
                            f"  {size.capitalize()}: {stats['mean']:.2f}x average speedup ({stats['count']} scenarios)\n"
                        )
    ...
    def _generate_recommendations(self, results: dict) -> str:
        ...
        if parallel_mean < 1.1:
            recommendations.append(
                ...
                "Consider disabling for small GIFs to reduce overhead.".format(
                    f"{parallel_mean:.2f}"
                )
            )
            ...
            recommendations.append(
                ...
                "Ensure it's enabled for production workloads.".format(
                    f"{parallel_mean:.2f}"
                )
            )
        ...
        if conditional_mean > 1.3:
            recommendations.append(
                ...
                "This should be the primary optimization strategy.".format(
                    f"{conditional_mean:.2f}"
                )
            )
        ...
        if (
            size_speedups.get("large", {}).get("mean", 0)
            > size_speedups.get("small", {}).get("mean", 0) * 1.5
        ):
            recommendations.append(
...
def main():
    ...
    if summary:
        print(
            f"Mean Speedup (Full Optimization): {summary['full_optimization']['mean_speedup']:.2f}x"
        )
        print(f"Best Case Speedup: {summary['full_optimization']['max_speedup']:.2f}x")
        print(
            f"Mean Memory Reduction: {summary['memory_optimization']['mean_reduction']:.1f}%"
        )
