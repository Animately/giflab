...
class FrameCacheBenchmark:
    ...
    def benchmark_single_file(
        self, gif_path: Path, iterations: int = 10, max_frames: int | None = None
    ) -> dict[str, float]:
        ...
        return {
            ...
            "cached_stdev": statistics.stdev(times_with_cache)
            ...
            "uncached_stdev": statistics.stdev(times_without_cache)
            if len(times_without_cache) > 1
            else 0,
            "uncached_min": min(times_without_cache),
            "uncached_max": max(times_without_cache),
            "speedup": statistics.mean(times_without_cache)
            / statistics.mean(times_with_cache),
    ...
    def benchmark_cache_size_impact(
        self, test_files: dict[str, Path], memory_limits_mb: list[int] = None
    ) -> list[dict]:
        ...
        for limit_mb in memory_limits_mb:
            ...
            limit_results = {"memory_limit_mb": limit_mb, "files": {}}
    ...
    def benchmark_concurrent_access(
        self, gif_path: Path, num_threads: int = 10, iterations_per_thread: int = 5
    ) -> dict:
    ...
    def benchmark_cache_warming(
        self, test_files: list[Path], max_frames: int = 30
    ) -> dict:
    ...
    def run_full_benchmark(self, temp_dir: Path) -> None:
        ...
        memory_results = self.benchmark_cache_size_impact(
            test_files, memory_limits_mb=[10, 50, 100, 500]
        )
        ...
        concurrent_result = self.benchmark_concurrent_access(
            test_files["medium"], num_threads=10, iterations_per_thread=5
        )
        ...
        warm_result = self.benchmark_cache_warming(
            list(test_files.values()), max_frames=30
        )
    ...
    def print_summary(self) -> None:
        ...
        if "individual_files" in self.results:
            ...
            for name, result in self.results["individual_files"].items():
                ...
                print(
                    f"  {name:10s}: {speedup:5.1f}x speedup "
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Benchmark frame cache performance")
    parser.add_argument(
        "--output",
        type=Path,
