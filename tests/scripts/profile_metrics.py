...
def profile_frame_metrics(
    ...
    for orig_frame, comp_frame in sample_pairs:
        ...
        timings.setdefault("sharpness_similarity", []).append(
...
def main():
    ...
    compressed_path = (
        test_dir / "solid_dot_32x32_10frames_100ms.gif"
    )  # Use same file for profiling
    ...
    if not original_path.exists():
        ...
        frames = [
            np.random.randint(0, 255, (256, 256, 3), dtype=np.uint8) for _ in range(30)
        ]
    ...
    for metric, avg_time in sorted(
        avg_timings.items(), key=lambda x: x[1], reverse=True
    ):
        per_frame_time = avg_time * len(aligned_pairs)
        total_sequential_time += per_frame_time
        print(
            f"{metric:20s}: {avg_time*1000:7.3f}ms/frame | Total: {per_frame_time:.3f}s"
        )
