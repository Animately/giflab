...
class TestAllFixturesGenerated:
    ...
    def test_all_outputs_are_valid_gifs(self, tmp_path):
        ...
        assert bad == [], "Invalid GIF outputs:\n" + "\n".join(bad)
...
class TestReferencedFixturesAutoGenerated:
    """The root conftest must auto-generate the gitignored fixtures that tests
    reference, so a fresh checkout / CI runner (which never runs `make fixtures`)
    doesn't fail with "File not found". Regression guard for the
    missing-test_4_frames.gif class of failure.
    """

    @pytest.mark.parametrize("name", VALIDATION_FIXTURES)
    def test_referenced_fixture_present_in_repo(self, name):
        # conftest._ensure_referenced_canonical_fixtures() runs at import time, so
        # by the time any test executes these must already exist on disk.
        fixture = FIXTURES_DIR / name
        assert fixture.exists(), (
            f"{name} is referenced by tests but absent from {FIXTURES_DIR}. "
            "The root conftest auto-gen hook should have created it — see "
            "tests/conftest.py::_ensure_referenced_canonical_fixtures."
        )
