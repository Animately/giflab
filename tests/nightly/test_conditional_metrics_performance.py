...
class TestConditionalMetricsPerformance:
    ...
    def create_test_frames(self):
        ...
        def _create_frames(
            quality: str, num_frames: int = 10, size: tuple = (200, 200)
        ):
    ...
    def test_progressive_calculation_performance(self, create_test_frames):
        ...
        for quality_level, num_frames, max_time in test_cases:
            ...
            assert (
                elapsed < max_time
            ), f"{quality_level} quality took {elapsed:.3f}s, expected < {max_time}s"

        # Print performance summary
        print("\n=== Progressive Calculation Performance ===")
        for r in results:
            print(
                f"Quality: {r['quality']:8} | Frames: {r['frames']:3} | "
                f"Time: {r['time']:.4f}s | Max: {r['expected_max']:.2f}s"
            )
            if r["metadata"]:
                print(
                    f"  -> Tier: {r['metadata'].get('quality_tier', 'N/A'):8} | "
    ...
    def test_frame_hash_cache_performance(self):
        ...
        np.random.seed(42)
        frames = [
            np.random.randint(0, 256, (100, 100, 3), dtype=np.uint8) for _ in range(50)
        ]
    ...
    def test_optimization_scaling(
        self, create_test_frames, frame_count, expected_speedup
    ):
        ...
        non_optimized_time = (
            frame_count * 0.15
        )  # Assume 0.15s per frame for all metrics
...
class TestConditionalMetricsIntegration:
    ...
    def test_full_pipeline_comparison(self):
        ...
        num_frames = 20
        original = [
            np.ones((200, 200, 3), dtype=np.uint8) * (100 + i * 5)
            for i in range(num_frames)
        ]
        ...
        with patch(
            "giflab.metrics.calculate_comprehensive_metrics_from_frames"
        ) as mock_full:
