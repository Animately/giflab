...
class TestParallelMetricsBenchmark:
    ...
    def test_deterministic_results(self):
        ...
        compressed_frames = [
            np.clip(f.astype(np.float32) * 0.9, 0, 255).astype(np.uint8) for f in frames
        ]
        ...
        for metric in key_metrics:
            ...
            assert all(
                abs(v - values[0]) < 1e-6 for v in values
    ...
    def test_chunk_strategies(self):
        ...
        for strategy in strategies:
            ...
            results.append(
                {"strategy": strategy, "time": elapsed, "ssim": metrics.get("ssim", 0)}
            )
        ...
        assert all(
            abs(v - ssim_values[0]) < 1e-6 for v in ssim_values
    ...
    def test_memory_efficiency(self):
        ...
        # Memory increase should be reasonable (less than 12x frame data size)
        # Note: LPIPS model loading, intermediate arrays, and metric computation
        # allocate significant temporary memory beyond the raw frame data
        frame_data_size = sum(f.nbytes for f in frames) / 1024 / 1024
        assert (
            memory_increase < frame_data_size * 12
    ...
    def test_fallback_to_sequential(self, error_type):
        ...
        if error_type == "import_error":
            ...
            with mock.patch(
                "giflab.parallel_metrics.ParallelMetricsCalculator",
            ...
            with mock.patch(
                "giflab.parallel_metrics.ParallelMetricsCalculator.calculate_frame_metrics_parallel",
...
class TestParallelMetricsAccuracy:
    ...
    def test_metric_accuracy_vs_sequential(self):
        ...
        compressed_frames = [
            np.clip(
                f.astype(np.float32) * 0.9 + np.random.normal(0, 5, f.shape), 0, 255
            ).astype(np.uint8)
            for f in frames
        ]
