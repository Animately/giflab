...
class TestMemoryPressureManager:
    ...
    def setup_method(self):
        ...
        self.manager = MemoryPressureManager(
            self.system_monitor, self.cache_tracker, self.eviction_policy
        )
    ...
    def test_memory_pressure_check_no_pressure(self):
        ...
        with patch.object(
            self.system_monitor, "get_current_stats", return_value=normal_stats
        ):
    ...
    def test_memory_pressure_check_with_pressure(self):
        ...
        with patch.object(
            self.system_monitor, "get_current_stats", return_value=high_stats
        ):
            should_evict, target_mb = self.manager.check_memory_pressure()
...
class TestMemoryIntegration:
    ...
    def test_cache_instrumentation(self):
        ...
        with patch(
            "giflab.monitoring.memory_integration.get_cache_memory_tracker"
        ) as mock_tracker:
            with patch(
                "giflab.monitoring.memory_integration.get_memory_pressure_manager"
            ) as mock_manager:
                ...
                instrument_cache_with_memory_tracking(
                    "test_cache", size_callback, eviction_callback
                )
