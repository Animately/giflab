...
- The ceilings are **animately-calibrated only**. All four non-animately lossy engines have been calibrated (`scripts/audit/engine_lossy_calibration.py`; see [`docs/metrics-audit/2026-06-03/post-fix-verdict.md`](metrics-audit/2026-06-03/post-fix-verdict.md) §D) and **none currently has a ceiling**: `gifsicle` (2026-06-05), `gifski` (2026-06-09) and `ffmpeg` (recalibrated 2026-06-12 on its now-real palette/dither lossy axis — its pre-2026-06-12 `-q:v` mapping was inert for GIF output) all degrade *gradually* with `banding_score = 0` at every level (no posterisation cliff). `imagemagick` (recalibrated 2026-06-12; its old `-quality` mapping was likewise inert, now a real `-dither Riemersma -colors N` axis) keeps `lossy_level 0` byte-honest (plain re-save, composite 1.000) and degrades gradually *after* a one-off entry step at L0→10 (~0.24 composite on gradient content with banding 0 — its quantiser's cost of entering the axis, not progressive posterisation). Whether that entry step warrants a ceiling is deferred to the follow-up calibration task `giflab-imagemagick-lossy-entry-step-ceiling-calibration`; until that lands, the `engine == "animately"` gate stands.
