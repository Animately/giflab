...
- The ceilings are **animately-calibrated only**. All four non-animately lossy engines have been calibrated (`scripts/audit/engine_lossy_calibration.py`; see [`docs/metrics-audit/2026-06-03/post-fix-verdict.md`](metrics-audit/2026-06-03/post-fix-verdict.md) §D) and **none needs a ceiling** — for two different reasons: `gifsicle` (2026-06-05) and `gifski` (2026-06-09) have a real lossy axis that degrades *gradually* with `banding_score = 0` at every level (no posterisation cliff), while `ffmpeg` and `imagemagick` (2026-06-09) have an **inert** `lossy_level` for GIF output — their wrappers map it to `-q:v` / `-quality` (video-DCT / zlib knobs that don't touch GIF pixels), so output is byte-identical across all levels and no lossy axis is exercised. Either way the `engine == "animately"` gate is correct and fully data-backed; there is no remaining per-engine calibration TODO.
