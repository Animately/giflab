...
- The ceilings are **animately-calibrated only**. All four non-animately lossy engines have been calibrated (`scripts/audit/engine_lossy_calibration.py`; see [`docs/metrics-audit/2026-06-03/post-fix-verdict.md`](metrics-audit/2026-06-03/post-fix-verdict.md) §D) and **none currently has a ceiling**: `gifsicle` (2026-06-05), `gifski` (2026-06-09) and `ffmpeg` (recalibrated 2026-06-12 on its now-real palette/dither lossy axis — its pre-2026-06-12 `-q:v` mapping was inert for GIF output) all degrade *gradually* with `banding_score = 0` at every level (no posterisation cliff). `imagemagick` (recalibrated 2026-06-12; its old `-quality` mapping was likewise inert, now a real `-dither Riemersma -colors N` axis) keeps `lossy_level 0` byte-honest (plain re-save, composite 1.000) and degrades gradually *after* a one-off axis-entry cost — its quantiser's cost of entering the quantise+dither axis at all, not progressive posterisation. The entry-step calibration (closed same day, 2026-06-12) confirmed that cost is content-dependent (~0.24 composite on colour-dense gradients, zero on small well-separated palettes) and paid in full at the *first* positive level, even at the gentlest palette target — so no positive ceiling value could avoid it, and clamping to 0 would just disable lossy. After entry the curve is gradual with `banding_score = 0` throughout: **no imagemagick ceiling** (see the 2026-06-12 entry-step finding in `scripts/audit/engine_lossy_calibration.py`). The `engine == "animately"` gate stands, now fully data-backed for all four non-animately engines.
