    ...
    if _psutil_available is None:
        ...
        if _psutil_available:
            try:
                ...
                logger.warning(
                    "psutil import failed, memory monitoring will be limited"
                )
...
class EvictionPolicy(Protocol):
    ...
    def should_evict(
        self, memory_stats: MemoryStats, cache_usage: CacheMemoryUsage
    ) -> bool:
...
class ConservativeEvictionPolicy:
    ...
    def __init__(
        self, warning_threshold: float = 0.8, critical_threshold: float = 0.95
    ):
    ...
    def collect_memory_stats(self) -> MemoryStats:
        ...
        if self._psutil:
            try:
                ...
                process_memory_mb = process_info.rss / (
                    1024 * 1024
                )  # Resident Set Size
        ...
        try:
            ...
            if operation in operation_mapping:
                ...
                if hasattr(self, "_current_memory_stats"):
                    memory_pressure_level = getattr(
                        self._current_memory_stats, "pressure_level", None
                    )
            ...
            logger.debug(
                f"Failed to record cache operation for effectiveness monitoring: {e}"
            )
