...
class CacheRecommendation(Enum):
    ...
    ENABLE_WITH_MONITORING = (
        "enable_with_monitoring"  # Moderate evidence, monitor closely
    )
    ...
    PERFORMANCE_REGRESSION = (
        "performance_regression"  # Caching causing performance issues
    )
...
class CacheEffectivenessAnalyzer:
    ...
    def analyze_cache_effectiveness(self) -> CacheEffectivenessAnalysis:
        ...
        logger.info(
            f"Cache effectiveness analysis completed: {analysis.recommendation.value} (confidence: {analysis.confidence_score:.2f})"
        )
    ...
    def _perform_comprehensive_analysis(
        ...
        analysis = CacheEffectivenessAnalysis(
            recommendation=CacheRecommendation.INSUFFICIENT_DATA, confidence_score=0.0
        )
        ...
        avg_performance_improvement = baseline_report.get(
            "performance_analysis", {}
        ).get("average_improvement", 0.0)
        ...
        if not self._has_sufficient_data(
            cache_stats, baseline_stats, monitoring_duration
        ):
            ...
            analysis.optimization_recommendations = (
                self._get_insufficient_data_recommendations()
            )
            return analysis
        ...
        performance_analysis = self._analyze_performance_impact(
            baseline_stats, avg_performance_improvement
        )
        ...
        analysis.baseline_comparison_summary = self._summarize_baseline_comparisons(
            baseline_stats
        )
        ...
        analysis.suggested_cache_sizes = self._suggest_cache_sizes(
            cache_stats, memory_analysis
        )
        analysis.suggested_eviction_thresholds = self._suggest_eviction_thresholds(
            memory_analysis
        )
    ...
    def _has_sufficient_data(
        ...
        valid_baselines = sum(
            1 for stats in baseline_stats.values() if stats.min_samples_met
        )
    ...
    def _analyze_hit_rates(
        self, cache_stats: dict[str, CacheEffectivenessStats], overall_hit_rate: float
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "cache_type_hit_rates": {
                cache_type: stats.hit_rate for cache_type, stats in cache_stats.items()
            },
            "hit_rate_variance": statistics.stdev(hit_rates)
            if len(hit_rates) > 1
            else 0.0,
        }

    def _analyze_performance_impact(
        self, baseline_stats: dict[str, BaselineStatistics], avg_improvement: float
    ) -> dict[str, Any]:
        ...
        valid_comparisons = [
            stats for stats in baseline_stats.values() if stats.statistical_significance
        ]
        ...
        return {
            ...
            "improvement_variance": statistics.stdev(
                [stats.performance_improvement for stats in valid_comparisons]
            )
            if len(valid_comparisons) > 1
            else 0.0,
            "operation_improvements": {
                stats.operation_type: stats.performance_improvement
                for stats in valid_comparisons
            },
        }
    ...
    def _analyze_memory_efficiency(
        ...
        if (
            "effectiveness_monitoring" in memory_summary
            and memory_summary["effectiveness_monitoring"] == "disabled"
        ):
            return {
                "overall_assessment": "monitoring_disabled",
                "efficiency_score": 0.5,
            }

        # Calculate memory utilization metrics
        total_cache_memory = sum(
            stats.total_data_cached_mb for stats in cache_stats.values()
        )
    ...
    def _generate_overall_recommendation(
    ...
    ) -> tuple[CacheRecommendation, float]:
        ...
        overall_score = (
            (performance_score * 0.5) + (hit_rate_score * 0.35) + (memory_score * 0.15)
        )
    ...
    def _generate_optimization_recommendations(
        ...
        if overall_hit_rate < self.good_hit_rate_threshold:
            recommendations.append(
                f"Low hit rate ({overall_hit_rate:.1%}). Consider reviewing cache key strategies or increasing cache size."
            )
        ...
        if 0 < avg_improvement < self.acceptable_performance_improvement:
            recommendations.append(
                "Marginal performance improvement detected. Optimize cache implementation or evaluate cost/benefit."
            )

        # Memory optimization
        high_eviction_caches = [
            cache_type
            for cache_type, rate in memory_analysis.get(
                "cache_type_eviction_rates", {}
            ).items()
            if rate > 0.3
        ]
        if high_eviction_caches:
            recommendations.append(
                f"High eviction rates detected for: {', '.join(high_eviction_caches)}. Consider increasing cache sizes or optimizing eviction policies."
            )
        ...
        for cache_type, stats in cache_stats.items():
            if stats.cache_turnover_rate > 0.8:
                recommendations.append(
                    f"{cache_type}: High turnover rate ({stats.cache_turnover_rate:.1%}). Review cache size and TTL settings."
                )

            if stats.hit_rate < 0.3 and stats.total_operations > 100:
                recommendations.append(
                    f"{cache_type}: Very low hit rate ({stats.hit_rate:.1%}). Consider disabling or redesigning cache strategy."
                )
    ...
    def _generate_performance_insights(
        ...
        if monitoring_hours > 0:
            ...
            insights.append(
                f"Processing rate: {ops_per_hour:.0f} operations/hour over {monitoring_hours:.1f} hours"
            )
        ...
        if cache_stats:
            ...
            if best_hit_rate > 0:
                ...
                insights.append(
                    f"Best performing cache: {best_cache} ({best_hit_rate:.1%} hit rate)"
                )
            ...
            if best_improvement > 0:
                best_operation = next(
                    (
                        ...
                        for stats in baseline_stats.values()
                        if stats.performance_improvement == best_improvement
                    ),
                    None,
                )
                insights.append(
                    f"Highest performance gain: {best_operation} ({best_improvement:.1%} improvement)"
                )
    ...
    def _identify_risk_factors(
        ...
        if memory_analysis.get("efficiency_score", 0.0) < 0.4:
            risks.append(
                "High memory pressure detected. Cache evictions may impact performance."
            )

        # Performance regression risks
        if performance_analysis.get("average_improvement", 0.0) < 0:
            risks.append(
                "Negative performance impact detected. Caching may be adding overhead."
            )
        ...
        for cache_type, stats in cache_stats.items():
            if stats.hit_rate > 0.8 and stats.cache_turnover_rate < 0.1:
                risks.append(
                    f"{cache_type}: Very high hit rate with low turnover may indicate stale data issues."
                )

        # Insufficient baseline data
        if performance_analysis.get("valid_comparisons", 0) < 2:
            risks.append(
                "Limited baseline comparisons available. Recommendations may be less reliable."
            )

        return risks

    def _suggest_cache_sizes(
        self,
        cache_stats: dict[str, CacheEffectivenessStats],
        memory_analysis: dict[str, Any],
    ) -> dict[str, float]:
        ...
        for cache_type, stats in cache_stats.items():
            ...
            if eviction_rate > 0.5:  # High eviction rate - suggest larger cache
                ...
                eviction_rate < 0.1 and stats.hit_rate < 0.6
            ):  # Low eviction but poor hit rate - may be too large
    ...
    def _suggest_eviction_thresholds(
    ...
    def _summarize_cache_stats(
        self, cache_stats: dict[str, CacheEffectivenessStats]
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "total_operations": sum(
                stats.total_operations for stats in cache_stats.values()
            ),
            ...
            "cache_type_summaries": {
                cache_type: {
                    ...
                    "eviction_rate": stats.evictions / stats.puts
                    if stats.puts > 0
                    else 0.0,
    ...
    def _summarize_baseline_comparisons(
