...
class ValidationCache:
        ...
        with self._lock:
            ...
            if cache_key in self._memory_cache:
                ...
                if time.time() - entry.timestamp < self.ttl_seconds:
                    ...
                    self._stats.hits += 1
                    logger.debug(
                        f"Cache hit (memory): {metric_type} for frames {frame_indices}"
                    )
                    return entry.value
                else:
            ...
            if cached_result:
                ...
                if time.time() - cached_result.timestamp < self.ttl_seconds:
                    ...
                    self._stats.hits += 1
                    logger.debug(
                        f"Cache hit (disk): {metric_type} for frames {frame_indices}"
                    )
    ...
    def put(
        ...
        with self._lock:
            ...
            # Store to disk
            self._store_to_disk(cache_key, entry, frame1_hash, frame2_hash)
            ...
            with sqlite3.connect(str(self.disk_path)) as conn:
                ...
                """,
                    (
                        ...
                        frame1_hash,
                        frame2_hash,
                        json.dumps(entry.frame_indices)
