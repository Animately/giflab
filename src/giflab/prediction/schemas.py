"""Pydantic schemas for Compression Curve Prediction.
...
class GifFeaturesV1(BaseModel):
    ...
    unique_colors: int = Field(
        ...
        ge=1,
        le=256,
        description="Unique colors in palette",
    )
    ...
    edge_density: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Edge pixel ratio",
    )
    color_complexity: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Color distribution complexity",
    )
    gradient_smoothness: float = Field(
        ...,
        ge=0.0,
        le=1.0,
    ...
    contrast_score: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Image contrast level",
    )
    text_density: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Text/UI element density",
    )
    dct_energy_ratio: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="High-freq / low-freq DCT energy",
    )
    color_histogram_entropy: float = Field(
        ...,
        ge=0.0,
        le=8.0,
        description="Color usage distribution",
    )
    dominant_color_ratio: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Top-10 colors as % of pixels",
    )
    ...
    motion_intensity: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Average frame-to-frame change",
    )
    motion_smoothness: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Motion consistency",
    )
    static_region_ratio: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Unchanging pixel ratio",
    )
    ...
    frame_similarity: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Average inter-frame similarity",
    )
    inter_frame_mse_mean: float = Field(
        ...,
        ge=0.0,
        description="Mean squared error between frames",
    )
    inter_frame_mse_std: float = Field(
        ...,
        ge=0.0,
        description="MSE standard deviation",
    )

    # Compressibility
    lossless_compression_ratio: float = Field(
        ...,
        ge=0.0,
        le=2.0,
        description="Size after lossless / original",
    )
...
class CompressionCurveV1(BaseModel):
    ...
    is_predicted: bool = Field(
        ...,
        description="True if predicted, False if actual",
    )
    model_version: str | None = Field(
        None,
        description="Prediction model version (if predicted)",
    )
    confidence_scores: list[float] | None = Field(
        None,
        description="Confidence per point (if predicted)",
    )
    ...
    size_at_colors_256: float | None = Field(
        None,
        gt=0,
        description="File size KB at 256 colors",
    )
    size_at_colors_128: float | None = Field(
        None,
        gt=0,
        description="File size KB at 128 colors",
    )
    size_at_colors_64: float | None = Field(
        None,
        gt=0,
        description="File size KB at 64 colors",
    )
    size_at_colors_32: float | None = Field(
        None,
        gt=0,
        description="File size KB at 32 colors",
    )
    size_at_colors_16: float | None = Field(
        None,
        gt=0,
        description="File size KB at 16 colors",
    )
    ...
    def get_color_curve_points(self) -> dict[int, float | None]:
        """Return color curve as dict of count -> size."""
        return {
            256: self.size_at_colors_256,
            128: self.size_at_colors_128,
            64: self.size_at_colors_64,
            32: self.size_at_colors_32,
            16: self.size_at_colors_16,
        }
...
class TrainingRecordV1(BaseModel):
    ...
    lossy_curve_gifsicle: CompressionCurveV1 = Field(
        ...,
        description="Actual gifsicle lossy curve",
    )
    lossy_curve_animately: CompressionCurveV1 | None = Field(
        None,
        description="Actual animately lossy curve",
    )
    color_curve_gifsicle: CompressionCurveV1 = Field(
        ...,
        description="Actual gifsicle color curve",
    )
    color_curve_animately: CompressionCurveV1 | None = Field(
        None,
        description="Actual animately color curve",
    )
...
class PredictionModelMetadataV1(BaseModel):
    ...
    training_dataset_version: str = Field(
        ...,
        pattern=r"^\d+\.\d+\.\d+$",
        description="Dataset used for training",
    )
    training_samples: int = Field(..., gt=0, description="Training sample count")
    ...
    giflab_version: str = Field(
        ...,
        pattern=r"^\d+\.\d+\.\d+$",
        description="GifLab version at training",
    )
