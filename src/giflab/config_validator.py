...
class ConfigurationValidator:
    ...
    def _register_default_rules(self):
        ...
        self.add_rule(
            "FRAME_CACHE.enabled",
            ValidationRule("type", TypeValidator.is_bool, "enabled must be a boolean"),
        )

        self.add_rule(
            "FRAME_CACHE.memory_limit_mb",
            ValidationRule(
                "type", TypeValidator.is_int, "memory_limit_mb must be an integer"
            ),
        )
        self.add_rule(
            "FRAME_CACHE.memory_limit_mb",
            ValidationRule(
                "range",
                RangeValidator.in_range(10, 8192),
                "memory_limit_mb must be between 10 and 8192",
            ),
        )

        self.add_rule(
            "FRAME_CACHE.disk_limit_mb",
            ValidationRule(
                "positive", RangeValidator.positive, "disk_limit_mb must be positive"
            ),
        )

        self.add_rule(
            "FRAME_CACHE.ttl_seconds",
            ValidationRule(
                "range",
                RangeValidator.in_range(60, 604800),
        ...
        self.add_rule(
            "VALIDATION_CACHE.memory_limit_mb",
            ValidationRule(
                "range",
                RangeValidator.in_range(10, 2048),
        ...
        self.add_rule(
            "FRAME_SAMPLING.enabled",
            ValidationRule("type", TypeValidator.is_bool, "enabled must be a boolean"),
        )

        self.add_rule(
            "FRAME_SAMPLING.min_frames_threshold",
            ValidationRule(
                "range",
                RangeValidator.in_range(1, 1000),
                "min_frames_threshold must be between 1 and 1000",
            ),
        )

        self.add_rule(
            "FRAME_SAMPLING.confidence_level",
            ValidationRule(
                "range",
                RangeValidator.in_range(0.5, 0.99),
                "confidence_level must be between 0.5 and 0.99",
            ),
        )

        self.add_rule(
            "FRAME_SAMPLING.default_strategy",
            ValidationRule(
                "enum",
                lambda x: x
                in ["uniform", "adaptive", "progressive", "scene_aware", "full"],
        ...
        self.add_rule(
            "MONITORING.buffer_size",
            ValidationRule(
                "range",
                RangeValidator.in_range(100, 100000),
                "buffer_size must be between 100 and 100000",
            ),
        )

        self.add_rule(
            "MONITORING.sampling_rate",
            ValidationRule(
                "percentage",
                RangeValidator.percentage,
        ...
        # Engine path rules
        self.add_rule(
            "engine.GIFSICLE_PATH",
            ValidationRule(
                "executable",
                ResourceValidator.executable_exists,
        ...
        self.add_rule(
            "paths.RAW_DIR",
            ValidationRule(
                "type", TypeValidator.is_path, "RAW_DIR must be a valid path"
            ),
        )
        ...
        self.add_rule(
            ...
            ValidationRule(
                "writable",
                ResourceValidator.directory_writable,
    ...
    def validate(self, config: dict[str, Any]) -> dict[str, list[str]]:
        ...
        for path, rules in self.rules.items():
            ...
            for rule in rules:
                ...
                if (
                    hasattr(rule.validator, "__code__")
                    and rule.validator.__code__.co_argcount > 1
                ):
    ...
    def validate_relationships(self, config: dict[str, Any]) -> list[str]:
        ...
        if total_cache_memory > 4096:
            errors.append(
                f"Total cache memory ({total_cache_memory}MB) exceeds "
        ...
        if "FRAME_SAMPLING" in config:
            ...
            if sampling.get("enabled"):
                ...
                if strategy and strategy != "full" and strategy not in sampling:
                    errors.append(
                        f"Missing configuration for sampling strategy: {strategy}"
                    )
        ...
        if "metrics" in config:
            ...
            if metrics.get("USE_ENHANCED_COMPOSITE_QUALITY"):
                ...
                if abs(total - 1.0) > 1e-6:
                    errors.append(
                        f"Enhanced metric weights must sum to 1.0, got {total}"
                    )
    ...
    def check_resources(self, config: dict[str, Any]) -> list[str]:
        ...
        try:
            ...
            if total_configured > available_mb * 0.5:
                warnings.append(
                    f"Configured cache memory ({total_configured}MB) exceeds "
        ...
        for path, limit_mb in cache_paths:
            if not ResourceValidator.disk_space_available(path, limit_mb):
                warnings.append(
                    f"Insufficient disk space for cache at {path} "
                    f"(needs {limit_mb}MB)"
                )
